...
def parse_args():
    ...
    parser.add_argument(
        "--pred_2D",
        type=str,
        default="/netscratch/muhammad/codes/gen2seg/results/Fluo-N3DH-SIM+/2DResults/stable_diffusion_fluo_with_bg_Vanilla/Epoch-24"
    )

    parser.add_argument(
        "--root_directory",
        type=str,
        default="/netscratch/muhammad/codes/gen2seg/results/Fluo-N3DH-SIM+/3DResults/stable_diffusion_fluo_with_bg_Vanilla/Epoch-24"
    )
...
def main():
    ...
    for time_step, image_range in zip(time_steps, image_ranges):
        ...
        for image_num in tqdm(image_range, desc=f"Processing images for {time_step}"):
            ...
            tiff.imwrite(output_path, result_3d)
